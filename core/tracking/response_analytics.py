...
class ResponseMetrics:
    ...
    response_distribution: Dict[str, int]  # positive, negative, neutral, question, opt_out
...
class ResponseAnalytics:
    ...
    def get_response_metrics(self, campaign_id: Optional[str] = None, 
                           days: int = 30) -> ResponseMetrics:
        ...
        try:
            ...
            # Response rate
            response_rate = (total_responses / total_messages * 100) if total_messages > 0 else 0.0
            ...
            response_distribution = self._calculate_response_distribution(responded_messages)
            
            # Sentiment distribution
            sentiment_distribution = self._calculate_sentiment_distribution(responded_messages)
            
            # Peak response hours
            peak_response_hours = self._calculate_peak_response_hours(responded_messages)
    ...
    def get_response_heatmap(self, campaign_id: Optional[str] = None, 
    ...
    def get_response_funnel(self, campaign_id: str) -> Dict[str, int]:
        ...
        try:
            ...
            funnel = {
                "sent": len(messages),
                "delivered": len([msg for msg in messages if msg.delivery_status in ["delivered", "read"]]),
            ...
            funnel["delivery_rate"] = (funnel["delivered"] / funnel["sent"] * 100) if funnel["sent"] > 0 else 0
            funnel["read_rate"] = (funnel["read"] / funnel["delivered"] * 100) if funnel["delivered"] > 0 else 0
            funnel["response_rate"] = (funnel["responded"] / funnel["read"] * 100) if funnel["read"] > 0 else 0
    ...
    def get_response_patterns(self, campaign_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            response_lengths = [len(msg.response_content or "") for msg in responded_messages]
            ...
            all_responses = " ".join([msg.response_content or "" for msg in responded_messages]).lower()
    ...
    def _is_cache_valid(self, cache_key: str) -> bool:
        ...
        age_minutes = (datetime.now(timezone.utc) - self._cache_timestamp).total_seconds() / 60
        return age_minutes < self._cache_ttl_minutes and cache_key in self._cache
    
    def _calculate_response_distribution(self, responded_messages: List) -> Dict[str, int]:
    ...
    def _calculate_sentiment_distribution(self, responded_messages: List) -> Dict[str, int]:
    ...
    def _get_most_common_responses(self, responded_messages: List, limit: int = 10) -> List[Tuple[str, int]]:
    ...
    def _calculate_response_trend(self, messages: List, days: int) -> List[Tuple[str, float]]:
    ...
    def _analyze_content_performance(self, messages: List) -> Dict[str, float]:
        ...
        for msg in messages:
            content = msg.message_content[:100]  # Use first 100 chars as content identifier
    ...
    def _calculate_response_quality_score(self, messages: List) -> float:
        ...
        sentiments = [msg.sentiment_score for msg in responded_messages if msg.sentiment_score is not None]
        ...
        avg_response_time = statistics.mean(response_times) if response_times else 60  # Default 1 hour
        time_score = max(0, 100 - avg_response_time)  # Faster = higher score
        
        # Combine factors
        quality_score = (response_rate * 0.4) + ((avg_sentiment + 1) * 50 * 0.3) + (time_score * 0.3)
    ...
    def _determine_engagement_level(self, messages: List) -> str:
        ...
        response_rate = len([msg for msg in messages if msg.response_received]) / len(messages) * 100
    ...
    def _generate_recommendations(self, messages: List, campaign_id: str) -> List[str]:
        ...
        if response_rate < 10:
            recommendations.append("Consider reviewing message content - very low response rate")
        elif response_rate < 25:
            recommendations.append("Try personalizing messages better to improve response rate")
        elif response_rate > 50:
            recommendations.append("Excellent response rate! Consider scaling this campaign")
        ...
        if hour_stats:
            hour_rates = {hour: (stats["responded"] / stats["sent"] * 100) if stats["sent"] > 0 else 0 
                         for hour, stats in hour_stats.items()}
            ...
            if hour_rates[best_hour] - hour_rates[worst_hour] > 20:
                recommendations.append(f"Consider sending more messages around {best_hour:02d}:00 for better response rates")
        
        # Content recommendations
        responded_messages = [msg for msg in messages if msg.response_received]
        if responded_messages:
            positive_responses = len([msg for msg in responded_messages 
                                   if msg.sentiment_score and msg.sentiment_score > 0.1])
            
            if positive_responses / len(responded_messages) < 0.3:
                recommendations.append("Many responses seem neutral or negative - consider adjusting message tone")
