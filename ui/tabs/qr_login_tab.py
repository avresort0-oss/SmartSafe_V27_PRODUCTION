...
class Tab(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        self.header.pack(fill="x", padx=SPACING["md"], pady=(SPACING["sm"], SPACING["xs"]))

        # Header Badge
        badge_colors = self._get_badge_colors_from_tone("neutral")
        self.header_status = StatusBadge(self.header.actions, text="Ready", **badge_colors)
        ...
        self.link_method_container = ctk.CTkFrame(link_card.inner_frame, fg_color="transparent")
        ...
        ctk.CTkLabel(self.qr_frame, text="Scan with WhatsApp", font=heading(TYPOGRAPHY["h3"], "bold")).pack(pady=(0, 0))
        
        self.qr_container = ctk.CTkFrame(self.qr_frame, fg_color="white", corner_radius=RADIUS["lg"], width=300, height=300)
        self.qr_container.pack(pady=SPACING["md"], padx=SPACING["lg"])
        self.qr_container.pack_propagate(False)

        self.qr_image_label = ctk.CTkLabel(self.qr_container, text="Initializing...", text_color="black", font=body(TYPOGRAPHY["body"]))
        self.qr_image_label.place(relx=0.5, rely=0.5, anchor="center")

        self.session_label = ctk.CTkLabel(self.qr_frame, text="Session expires in 10:00", text_color=COLORS["text_secondary"], font=body(TYPOGRAPHY["caption"]))
        self.session_label.pack(pady=(0, SPACING["md"]))

        # Instructions
        inst_frame = ctk.CTkFrame(self.qr_frame, fg_color=COLORS["surface_2"], corner_radius=RADIUS["md"])
        inst_frame.pack(fill="x", padx=SPACING["lg"], pady=(0, SPACING["lg"]))
        
        steps = ["1. Open WhatsApp on phone", "2. Menu > Linked Devices", "3. Tap 'Link a Device'", "4. Point camera at screen"]
        for step in steps:
             ctk.CTkLabel(inst_frame, text=step, font=body(TYPOGRAPHY["body"]), text_color=COLORS["text_primary"], anchor="w").pack(fill="x", padx=SPACING["md"], pady=4)

        # --- Pairing Code Frame (NEW, inside link_method_container) ---
        self.pairing_code_frame = ctk.CTkFrame(self.link_method_container, fg_color="transparent")
        # self.pairing_code_frame is hidden by default

        ctk.CTkLabel(self.pairing_code_frame, text="Enter this code on your phone", font=heading(TYPOGRAPHY["h3"], "bold")).pack(pady=(0, 0))

        # Manual Phone Input
        self.pairing_input_frame = ctk.CTkFrame(self.pairing_code_frame, fg_color="transparent")
        self.pairing_input_frame.pack(pady=(SPACING["sm"], 0))
        ctk.CTkLabel(self.pairing_input_frame, text="Phone Number:", font=body(TYPOGRAPHY["caption"])).pack(side="left", padx=5)
        self.pairing_phone_entry = StyledInput(self.pairing_input_frame, width=180, placeholder_text="e.g. 1234567890")
        ...
        pairing_inst_frame = ctk.CTkFrame(self.pairing_code_frame, fg_color=COLORS["surface_2"], corner_radius=RADIUS["md"])
        ...
        for step in pairing_steps:
             ctk.CTkLabel(pairing_inst_frame, text=step, font=body(TYPOGRAPHY["body"]), text_color=COLORS["text_primary"], anchor="w").pack(fill="x", padx=SPACING["md"], pady=4)
        ...
        ctk.CTkLabel(info_card.inner_frame, text="Status Details", font=heading(TYPOGRAPHY["h3"], "bold")).pack(anchor="w", pady=(0, SPACING["xs"]))
        self.info_labels = {}
        ...
        self.refresh_button = PrimaryButton(action_card.inner_frame, text="Refresh QR Code", command=self._on_refresh)
        self.refresh_button.pack(fill="x", pady=(0, SPACING["xs"]))
        self.save_qr_button = SecondaryButton(action_card.inner_frame, text="Save QR Image", command=self._on_save_qr)
        self.save_qr_button.pack(fill="x", pady=(0, SPACING["xs"]))
        SecondaryButton(action_card.inner_frame, text="Disconnect Session", command=self._on_disconnect, fg_color=COLORS["danger"], hover_color=COLORS["danger_hover"], text_color="white").pack(fill="x")
    ...
    def _on_link_mode_change(self, mode: str):
        ...
        if mode == "QR Code":
            ...
            self.save_qr_button.configure(state="disabled") # Can't save a pairing code as image
    ...
    def _create_info_row(self, parent, label_text, key):
        ...
        ctk.CTkLabel(f, text=label_text, text_color=COLORS["text_secondary"], font=body(TYPOGRAPHY["body"])).pack(side="left")
        v = ctk.CTkLabel(f, text="-", text_color=COLORS["text_primary"], font=body(TYPOGRAPHY["body"], "bold"))
        v.pack(side="right")
    ...
    def _load_accounts(self):
        ...
        try:
            ...
            if not result.get("ok"):
                ...
                ui_dispatch(self, lambda: self.header_status.configure(text="No Accounts", **badge_colors))
            ...
            if not accounts_data:
                ...
                ui_dispatch(self, lambda: self.header_status.configure(text="No Config", **badge_colors))
                return
    ...
    def _fetch_pairing_code(self, account_id: str):
        ...
        if not clean_number:
            return {"ok": False, "error": "Account must have a phone number to generate pairing code"}
    ...
    def _update_status_display(self):
        ...
        tone = "neutral"
        if status == ConnectionStatus.CONNECTED: tone = "success"
        elif status == ConnectionStatus.CONNECTING: tone = "warning"
        elif status == ConnectionStatus.ERROR: tone = "danger"
        elif status == ConnectionStatus.EXPIRED: tone = "warning"
    ...
    def _update_session_timer(self):
        ...
        if time_remaining.total_seconds() > 0:
             self.session_label.configure(text=f"Session expires in {minutes:02d}:{seconds:02d}")
    ...
    def _on_generate_pairing_code(self):
        ...
        def _work():
            ...
            if manual_phone:
                result = self.api.get_pairing_code(self.current_account.account_id, manual_phone)
            else:
    ...
    def _on_save_qr(self):
        ...
        if img:
            ...
            ui_dispatch(self, lambda: self.header_status.configure(text="Saved", **badge_colors))
        else:
            badge_colors = self._get_badge_colors_from_tone("warning")
            ui_dispatch(self, lambda: self.header_status.configure(text="No QR", **badge_colors))
